...
def packing_3d(data):
    try:
        ...
        for i in range(nr):
            ...
            for j in range(nr):
                if i != j:
                    # Determine when item j is directly above item i
                    ...
                    # Ensure weight of item j is less than or equal to fragility of item i
                    model.Add(wr[j] <= fr[i]).OnlyEnforceIf(above)

                    # Accumulate weight_above only when item j is above item i
                    model.Add(weight_above >= wr[j]).OnlyEnforceIf(above)
